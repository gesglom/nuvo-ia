...
from core.job_queue import create_job, get_job, list_jobs
...
class NuvoHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/api/agents":
            agents = load_agents()
            return self._send_json({"agents": sorted(list(agents.keys()))})
    ...
    def do_POST(self):
        ...
        if self.path == "/api/jobs":
            ...
            if not goal:
                return self._send_json({"error": "goal es requerido"}, status=HTTPStatus.BAD_REQUEST)
            agents = load_agents()
            plan = create_plan(goal=goal, agents=agents)
...
def create_plan(goal, agents):
    # wrapper para evitar ciclo de imports en runtime
